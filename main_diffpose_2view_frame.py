...
from runners.diffpose_frame_mhad_2view import Diffpose
...
def parse_args_and_config():
    ...
    parser.add_argument("--simu", action = 'store_true',
                        help="simulate the diffusion and denoise process")
    ...
    #test hyperparameter
    parser.add_argument('--test_times', default=1, type=int, metavar='N',
                    help='the number of test times')
    parser.add_argument('--test_timesteps', default=1, type=int, metavar='N',
                    help='the number of test time steps')
    parser.add_argument('--test_num_diffusion_timesteps', default=50, type=int, metavar='N',
                    help='the number of test times')

    args = parser.parse_args()
    args.log_path = os.path.join(args.exp, args.doc + "@" + datetime.now().strftime("%d.%m.%Y-%H:%M:%S"))
...
def main():
    ...
    try:
        ...
        if args.train:
            ...
            _, _ = runner.test_hyber(is_train=False)
    except Exception:
