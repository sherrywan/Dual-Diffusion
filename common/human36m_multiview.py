...
class Human36MMultiViewDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for camera_idx, camera_name in enumerate(self.labels['camera_names']):
            ...
            T_camera = -(shot_camera['R'].T)@shot_camera['t']
            retval_camera = Camera(shot_camera['R'], T_camera,
                                   shot_camera['t'], shot_camera['K'],
                                   shot_camera['dist'], camera_name)
            T_bino.append(T_camera)
            ...
            if self.pred_2d_kernel is not None:
                sample['pred_2d_kernel'].append(self.pred_2d_kernel[camera_idx])
        ...
        if self.keypoints_2d_pred is not None:
            if len(self.include_cameras)==3:
                if not miss_ca:
                    sample['pred_keypoints_2d'] = self.keypoints_2d_pred[idx][1:]
                if miss_ca and len(self.include_cameras)==3:
                    sample['pred_keypoints_2d'] = self.keypoints_2d_pred[idx][:3]
            else:
    ...
    def evaluate(self,
        ...
        # sym
        tans_T = trans_matric[np.newaxis, :, :].transpose((0,2,1))
        bone_pred = np.matmul(tans_T, keypoints_3d_predicted)
        bl_pred = np.sqrt((bone_pred**2).sum(2))
        per_pose_sym_error = (abs(bl_pred[:,2]-bl_pred[:,3]) + abs(bl_pred[:,4]-bl_pred[:,5]) \
            + abs(bl_pred[:,9]-bl_pred[:,10])+ abs(bl_pred[:,11]-bl_pred[:,13]) \
            + abs(bl_pred[:,12]-bl_pred[:,14])+ abs(bl_pred[:,0]-bl_pred[:,1]))/6
        ...
        if keypoints_3d_gt is None:
            result = {
                ...
                self.evaluate_using_per_pose_error(per_pose_bonelength_error,
                                                split_by_subject),
                'per_pose_sym_error':
                self.evaluate_using_per_pose_error(per_pose_sym_error,
                                                split_by_subject),
                'per_pose_error':
                self.evaluate_using_per_pose_error(per_pose_error,
                                                split_by_subject),
            ...
            result = {
            'per_pose_bonelength_error': {'Average':per_pose_bonelength_error.mean()},
            'per_pose_sym_error':{'Average':per_pose_sym_error.mean()},
    ...
    def JDR_2d(self, keypoints_2d_pred, keypoints_2d_gt):
        ...
        pred = unnormalize_screen_coordinates(pred, self.image_shape[0],  self.image_shape[1]).reshape(-1,17,2)
        gt = unnormalize_screen_coordinates(gt, self.image_shape[0],  self.image_shape[1]).reshape(-1,17,2)
        ...
        joint_detection_rate = np.sum(detected, axis=0) / np.float64(gt.shape[0])
