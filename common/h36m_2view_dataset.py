...
class Human36m2ViewDataset(MocapDataset):
    def __init__(self, path, remove_static_joints=True):
        super(Human36m2ViewDataset, self).__init__(skeleton=h36m_skeleton, fps=50)

        self._cameras = copy.deepcopy(h36m_cameras_extrinsic_params)
        # another kind camera parameters
        self.cameras_meta = h5py.File("/data1/share/dataset/human36m_multi-view/extra/una-dinosauria-data/h36m/cameras.h5",'r')
        cameras_meta = np.empty((11, 4),
            dtype=[('R', np.float32, (3, 3)), ('T', np.float32, (3, 1)),
                ('t', np.float32, (3, 1)), ('K', np.float32, (3, 3)),
                ('dist', np.float32, 5), ('H', np.float32, (3, 3))])
        for subject_idx, subject in enumerate(self._cameras.keys()):
            ...
            for i, cam in enumerate(cameras):
                ...
                if subject_idx < 4:
                    subject = 'S1'
                camera_params = self.cameras_meta[subject.replace('S', 'subject')]['camera%d' % (i+1)]                   
                # change to numpy
                camera_retval = cameras_meta[int(subject[1:])-1][i]
                ...
                # normalize 
                camera_retval['T'] = camera_retval['T']/1000
                camera_retval['t'] = camera_retval['t']/1000
                f_xy = normalize_screen_coordinates(np.array([camera_retval['K'][0,2], camera_retval['K'][1,2]]), w=cam['res_w'], h=cam['res_h'])
                camera_retval['K'][0,2], camera_retval['K'][1,2] = f_xy[0] , f_xy[1]
                camera_retval['K'][0,0]= camera_retval['K'][0,0]/ cam['res_w'] * 2.0
                camera_retval['K'][0,1]= camera_retval['K'][0,1]/ cam['res_w'] * 2.0
                camera_retval['K'][1,1]= camera_retval['K'][1,1]/ cam['res_w'] * 2.0 
        ...
        for subject, actions in data.items():
            self._data[subject] = {}
            shot_camera = cameras_meta[int(subject[1:])-1]
