...
LastEditTime: 2024-04-10 20:25:29
FilePath: /wxy/3d_pose/Diffpose/common/generators.py
...
class PoseGenerator_gmm_2view(Dataset):
    def __init__(self, poses_3d, poses_2d_gmm, actions, camerapara):
        assert poses_3d is not None
        ...
        self._actions = reduce(lambda x, y: x + y, actions)
        self._camerapara = np.concatenate(camerapara)
        self._kernel_n = self._poses_2d_gmm.shape[3]
    ...
    def __getitem__(self, index):
        ...
        # randomly select a kernel from gmm
        out_pose_2d_kernel = np.zeros([out_pose_2d_gmm.shape[0], out_pose_2d_gmm.shape[1], out_pose_2d_gmm.shape[3]])
        for i in range(out_pose_2d_gmm.shape[0]):
            for j in range(out_pose_2d_gmm.shape[1]):
                out_pose_2d_kernel[i,j] = out_pose_2d_gmm[i,j,np.random.choice(self._kernel_n, 1, p=out_pose_2d_gmm[i,j,:,0]).item()]
        
        # generate uvxyz and uvxyz noise scale
        kernel_mean = out_pose_2d_kernel[:,:,1:3].swapaxes(1,0).reshape(-1, 4)
        kernel_variance = out_pose_2d_kernel[:,:,3:].swapaxes(1,0).reshape(-1,4)
