...
class SublayerConnection(nn.Module):
    ...
    def forward(self, x, sublayer, visible=False):
        if visible:
            y = self.norm(x)
            y, attn = sublayer(y)
...
class GraAttenLayer(nn.Module):
    ...
    def forward(self, x, mask, visible=False):
        if visible:
            x, attn = self.sublayer[0](x, lambda x: self.self_attn(x, x, x, mask, visible=visible), visible=visible)
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None, visible=False):
