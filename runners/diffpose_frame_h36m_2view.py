...
from common.data_utils import fetch_me, fetch_me_2view, read_3d_data_2view, create_2d_data
...
class Diffpose(object):
    ...
    def train(self):
        ...
        if config.data.dataset == "human36m":
            ...
            data_loader = train_loader = data.DataLoader(
                a(poses_train, poses_train_2d, actions_train, camerapara_train),
                batch_size=config.training.batch_size, shuffle=True,\
                    num_workers=config.training.num_workers, pin_memory=True)
        else:
            raise KeyError('Invalid dataset')
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            ...
            for i, (targets_uvxyz, targets_noise_scale, targets_2d, targets_3d, _, cameras_proj) in enumerate(data_loader):
                data_time += time.time() - data_start
                step += 1

                # to cuda
                targets_noise_scale, targets_2d, targets_3d, cameras_proj = \
                    targets_noise_scale.to(self.device), targets_2d.to(self.device), targets_3d.to(self.device), cameras_proj.to(self.device)
                ...
                x_3d = unproj_2d_to_3d(x, cameras_proj)
                if self.config.data.root_relative:
                    x_3d_root = x_3d[:,:1]
                    x_3d = x_3d - x_3d_root
                output_3d = self.model_diff(x_3d, src_mask, t.float(), 0)
    ...
    def test_hyber(self, is_train=False):
        ...
        for i, (_, input_noise_scale, input_2d, targets_3d, input_action, cameras_proj) in enumerate(data_loader):
            data_time += time.time() - data_start

            input_noise_scale, input_2d, targets_3d, cameras_proj = \
                input_noise_scale.to(self.device), input_2d.to(self.device), targets_3d.to(self.device), cameras_proj.to(self.device)
            ...
            output_uvxyz = generalized_steps_2view(x, src_mask, seq, self.model_diff, self.betas, eta=self.args.eta, cameras_proj=cameras_proj, root_relatvie=self.config.data.root_relative)
            output_uvxyz = output_uvxyz[0][-1]            
            output_uvxyz = torch.mean(output_uvxyz.reshape(test_times,-1,17,4),0)
            output_xyz = unproj_2d_to_3d(output_uvxyz, cameras_proj)
            epoch_loss_3d_pos.update(mpjpe(output_xyz, targets_3d).item() * 1000.0, targets_3d.size(0))
            epoch_loss_3d_pos_re.update(mpjpe(output_xyz[:, :, :]-output_xyz[:, :1, :], targets_3d[:, :, :]-targets_3d[:, :1, :]).item() * 1000.0, targets_3d.size(0))
            epoch_loss_3d_pos_procrustes.update(p_mpjpe((output_xyz[:, :, :]-output_xyz[:, :1, :]).cpu().numpy(), (targets_3d[:, :, :]-targets_3d[:, :1, :]).cpu().numpy()).item() * 1000.0, targets_3d.size(0))\
            ...
            input_xyz = unproj_2d_to_3d(input_2d, cameras_proj)
            epoch_loss_3d_pos_original.update(mpjpe(input_xyz, targets_3d).item() * 1000.0, targets_3d.size(0))
            epoch_loss_3d_pos_re_original.update(mpjpe(input_xyz[:, :, :]-input_xyz[:, :1, :], targets_3d[:, :, :]-targets_3d[:, :1, :]).item() * 1000.0, targets_3d.size(0))
            epoch_loss_3d_pos_procrustes_original.update(p_mpjpe((input_xyz[:, :, :]-input_xyz[:, :1, :]).cpu().numpy(), (targets_3d[:, :, :]-targets_3d[:, :1, :]).cpu().numpy()).item() * 1000.0, targets_3d.size(0))\
